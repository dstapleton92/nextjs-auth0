...
describe("Transaction Store", async () => {
  describe("get", async () => {
    describe("with a string secret", async () => {
      ...
      it("should return null if no transaction cookie with a matching state exists", async () => {
        ...
        headers.append(
          "cookie",
          `__txn_incorrect-state=${encryptedCookieValue}`
        );
        const requestCookies = new RequestCookies(headers);
    ...
    describe("with an object secret", async () => {
      it("should use the state to return the decrypted cookie", async () => {
        const secretString = await generateSecret(32);
        const secret: SessionSecretConfig = {
          currentKid: "key1",
          allowedKeys: { key1: secretString }
        };
        const codeVerifier = oauth.generateRandomCodeVerifier();
        const nonce = oauth.generateRandomNonce();
        const state = oauth.generateRandomState();
